...
(function() {
    ...
    // ===== Prompt Engine: builds the final AI prompt from selections =====
    var PromptEngine = {
        /**
         * Sentence mode: fluent AI image generation prompt
         * Organized by role priority for natural flow
         */
        buildSentence: function(selections, lang) {
            var useEn = (lang === 'en' || lang === 'both');
            var useCn = (lang === 'cn' || lang === 'both');

            // Group by role
            var groups = {};
            selections.forEach(function(s) {
                var role = s.role || 'misc';
            ...
            // Subject (dim 1) — the hospital building / department
            if (groups.subject && groups.subject.length > 0) {
                if (useEn) {
                    var subjectEn = groups.subject.map(function(s) { return s.en; }).join(', ');
                    parts.push(subjectEn);
                }
                if (useCn && lang === 'cn') {
                    var subjectCn = groups.subject.map(function(s) { return s.cn || s.en; }).join('、');
                    parts.push(subjectCn);
                }
            }

            // Layout / volume (dim 13)
            if (groups.layout && groups.layout.length > 0) {
                var layoutItems = groups.layout.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(layoutItems.join(', '));
            }

            // Space type (dim 2) — interior / exterior
            if (groups.space && groups.space.length > 0) {
                var spaceItems = groups.space.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(spaceItems.join(', '));
            }

            // Style (dim 3)
            if (groups.style && groups.style.length > 0) {
                var styleItems = groups.style.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(styleItems.join(', '));
            }

            // Material (dim 4)
            if (groups.material && groups.material.length > 0) {
                var matItems = groups.material.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(matItems.join(', '));
            }

            // Color (dim 5)
            if (groups.color && groups.color.length > 0) {
                var colorItems = groups.color.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(colorItems.join(', '));
            }

            // Interior lighting/material (dim 15)
            if (groups.interior && groups.interior.length > 0) {
                var intItems = groups.interior.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(intItems.join(', '));
            }

            // Light / time (dim 6)
            if (groups.light && groups.light.length > 0) {
                var lightItems = groups.light.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(lightItems.join(', '));
            }

            // View / camera (dim 7)
            if (groups.view && groups.view.length > 0) {
                var viewItems = groups.view.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(viewItems.join(', '));
            }

            // Context / surroundings (dim 8)
            if (groups.context && groups.context.length > 0) {
                var ctxItems = groups.context.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(ctxItems.join(', '));
            }

            // Mood / atmosphere (dim 9)
            if (groups.mood && groups.mood.length > 0) {
                var moodItems = groups.mood.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(moodItems.join(', '));
            }

            // Furniture / equipment (dim 11)
            if (groups.furniture && groups.furniture.length > 0) {
                var furnItems = groups.furniture.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(furnItems.join(', '));
            }

            // Landscape / nature (dim 12)
            if (groups.landscape && groups.landscape.length > 0) {
                var landItems = groups.landscape.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(landItems.join(', '));
            }

            // Detail (dim 14)
            if (groups.detail && groups.detail.length > 0) {
                var detItems = groups.detail.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(detItems.join(', '));
            }

            // Render params (dim 10)
            if (groups.render_param && groups.render_param.length > 0) {
                var renderItems = groups.render_param.map(function(s) { return useEn ? s.en : (s.cn || s.en); });
                parts.push(renderItems.join(', '));
            }

            // Join with commas, separating clauses with periods
            var prompt = parts.join(', ');

            // For English mode, add "Architectural visualization" prefix if not empty
            if (useEn && prompt.length > 0 && lang !== 'cn') {
                prompt = 'Architectural visualization of ' + prompt;
            }

            return prompt || 'No prompt generated';
        },

        /**
         * Keyword mode: comma-separated list of tags (for tools like SD/ComfyUI)
         */
        buildKeywordPhrase: function(selections, lang) {
            var parts = [];
            selections.forEach(function(s) {
                if (lang === 'en' || lang === 'both') {
                    parts.push(s.en);
                } else if (lang === 'cn') {
                    parts.push(s.cn || s.en);
                }
            });
            return parts.join(', ');
        }
